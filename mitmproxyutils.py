# This script is intended to be run by mitmproxy
# Usage: mitmdump -s mitmproxyutils.py -p 7934
...
# Discord webhook (disabled for now - add your webhook URL here when needed)
DISCORD_WEBHOOK_URL = ""
            ...
            if uid_from_response is not None:
                if not is_uid_whitelisted(uid_from_response):
                    flow.response.content = (
                        ...
                        f"\n"
                        f"[FFAA00]Contact [00FF00]Mani 272[FFAA00] For Access\n"
                        f"\n"
                    ).encode()
